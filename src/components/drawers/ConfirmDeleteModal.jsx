      ...
      {Boolean(showDeleteConfirm) && (
        <React.Fragment>
          <div onClick={deleteConfirmCancel} style={{ position: 'fixed', inset: 0, background: 'rgba(31,8,20,.5)', backdropFilter: 'blur(3px)', zIndex: 190, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 28 }}>
            <div onClick={stop} className="blscroll" style={{ width: '100%', maxWidth: 440, maxHeight: '85vh', overflowY: 'auto', background: '#fff', borderRadius: 22, boxShadow: 'var(--shadow-xl)', animation: 'blrise .3s var(--ease-out)' }}>
              <div style={{ padding: '22px 26px 4px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                ...
                <div>
                  <h3 style={{ fontFamily: "'Sora'", fontWeight: 700, fontSize: 18, color: 'var(--beet-700)', margin: 0 }}>{deleteConfirmTitle}</h3>
                </div>
              </div>
              ...
              <div style={{ padding: '16px 26px', borderTop: '1px solid var(--line-200)', display: 'flex', justifyContent: 'flex-end', gap: 10, position: 'sticky', bottom: 0, background: '#fff', borderRadius: '0 0 22px 22px' }}>
                <button onClick={deleteConfirmCancel} style={{ padding: '10px 18px', border: '1px solid var(--line-300)', background: '#fff', borderRadius: 11, fontSize: 13.5, fontWeight: 600, cursor: 'pointer', color: 'var(--ink-700)' }}>Cancel</button>
