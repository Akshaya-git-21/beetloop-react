    ...
    <React.Fragment>
      {Boolean(cvOpen) && (
        <div onClick={cvClose} style={{ position: 'fixed', inset: 0, zIndex: 170, background: 'rgba(31,8,20,.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 28 }}>
          <div onClick={cvStop} className="blscroll" style={{ width: '100%', maxWidth: 620, maxHeight: '100%', background: '#fff', borderRadius: 20, boxShadow: 'var(--shadow-xl)', overflowY: 'auto', animation: 'blrise .28s var(--ease-out)' }}>
            <div style={{ position: 'sticky', top: 0, background: '#fff', borderBottom: '1px solid var(--line-200)', padding: '18px 22px', zIndex: 2 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                <div>
                  ...
                  <h3 style={{ fontFamily: "'Sora'", fontWeight: 700, fontSize: 18, color: 'var(--beet-700)', margin: '4px 0 0' }}>Convert to tasks</h3>
                  <div style={{ fontSize: 12.5, color: 'var(--ink-500)', marginTop: 3 }}>{cvIdeaTitle}</div>
                </div>
                <button onClick={cvClose} style={{ width: 32, height: 32, borderRadius: 10, border: '1px solid var(--line-300)', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div style={{ padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <select value={f.planId || ''} onChange={cvSetPlan} style={{ width: '100%', minWidth: 0, padding: '10px 12px', border: '1px solid var(--line-300)', borderRadius: 11, fontSize: 12.5, background: '#fff' }}>
                    {(cvPlanOptions || []).map((o, oi) => <option key={oi} value={o.v}>{o.label}</option>)}
                  </select>
                  <select value={f.rowType || ''} onChange={cvSetRow} style={{ width: '100%', minWidth: 0, padding: '10px 12px', border: '1px solid var(--line-300)', borderRadius: 11, fontSize: 12.5, background: '#fff' }}>
              ...
              <div>
                ...
                {Boolean(cvKpiExisting) && (
                  <select value={f.kpiId || ''} onChange={cvSetKpi} style={{ width: '100%', minWidth: 0, padding: '10px 12px', border: '1px solid var(--line-300)', borderRadius: 11, fontSize: 12.5, background: '#fff' }}>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '.6fr 1fr', gap: 8, marginBottom: 8 }}>
                  ...
                  <select value={f.assignee || ''} onChange={cvSetAssignee} style={{ minWidth: 0, padding: '10px 12px', border: '1px solid var(--line-300)', borderRadius: 11, fontSize: 12.5, background: '#fff' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.2fr', gap: 8 }}>
                  ...
                  <select value={f.reviewer || ''} onChange={cvSetReviewer} style={{ minWidth: 0, padding: '10px 12px', border: '1px solid var(--line-300)', borderRadius: 11, fontSize: 12.5, background: '#fff' }}>
            ...
            <div style={{ position: 'sticky', bottom: 0, background: '#fff', padding: '14px 22px', borderTop: '1px solid var(--line-200)', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button onClick={cvClose} style={{ padding: '10px 17px', border: '1px solid var(--line-300)', background: '#fff', borderRadius: 11, fontSize: 13, fontWeight: 700, color: 'var(--ink-700)', cursor: 'pointer' }}>Cancel</button>
