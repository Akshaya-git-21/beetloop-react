    ...
    <React.Fragment>
      {Boolean(contentImportOpen) && (
        <div onClick={contentImportClose} style={{ position: 'fixed', inset: 0, zIndex: 180, background: 'rgba(31,8,20,.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 28 }}>
          <div onClick={contentImportStop} style={{ width: '100%', maxWidth: 460, background: '#fff', borderRadius: 20, boxShadow: 'var(--shadow-xl)', animation: 'blrise .28s var(--ease-out)' }}>
            <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--line-200)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
              <div>
                ...
                <h3 style={{ fontFamily: "'Sora'", fontWeight: 700, fontSize: 19, color: 'var(--beet-700)', margin: '4px 0 0' }}>Import pages from CSV</h3>
              </div>
              <button onClick={contentImportClose} style={{ width: 32, height: 32, borderRadius: 10, border: '1px solid var(--line-300)', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div style={{ padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 13 }}>
              ...
              <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, border: '1.5px dashed var(--line-300)', borderRadius: 14, padding: '26px 16px', cursor: 'pointer', textAlign: 'center' }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--beet-700)' }}>{contentImportFileName || 'Choose a .csv file'}</span>
            ...
            <div style={{ padding: '14px 22px', borderTop: '1px solid var(--line-200)', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button onClick={contentImportClose} style={{ padding: '10px 17px', border: '1px solid var(--line-300)', background: '#fff', borderRadius: 11, fontSize: 13, fontWeight: 700, color: 'var(--ink-700)', cursor: 'pointer' }}>Cancel</button>
