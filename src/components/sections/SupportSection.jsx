    ...
    <React.Fragment>
      {Boolean(supIsList) && (
        <React.Fragment>
          ...
          <div style={{ background: '#fff', border: '1px solid var(--line-300)', borderRadius: 18, boxShadow: 'var(--shadow-sm)', padding: '16px 18px', marginBottom: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(230px,1fr))', gap: 10 }}>
              {(supCatCards || []).map((c, i) => (
                <button key={i} onClick={c.pick} style={{ textAlign: 'left', padding: '13px 15px', border: '1px solid var(--line-300)', background: '#fff', borderRadius: 13, cursor: 'pointer', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
            ...
            {(supFilters || []).map((f, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, background: '#fff', border: '1px solid var(--line-300)', borderRadius: 11, padding: '6px 10px' }}>
                ...
                <select value={f.value} onChange={f.onChange} style={{ border: 'none', background: 'none', fontSize: 12.5, fontWeight: 600, color: 'var(--beet-700)', outline: 'none', cursor: 'pointer', maxWidth: 180 }}>
                  {(f.options || []).map(o => <option key={o} value={o}>{o}</option>)}
                </select>
              </div>
            ))}
            <button onClick={supReset} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', border: '1px solid var(--line-300)', background: '#fff', borderRadius: 11, fontSize: 12, fontWeight: 700, color: 'var(--ink-500)', cursor: 'pointer' }}>
                    ...
                    <tr key={t.id}>
                      <td onClick={t.open} style={{ padding: '12px 18px', borderBottom: '1px solid var(--line-200)', cursor: 'pointer', minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontFamily: "'Space Mono'", fontSize: 10.5, fontWeight: 700, color: 'var(--beet-700)' }}>{t.id}</span>
                      ...
                      <td style={{ padding: '12px 18px', borderBottom: '1px solid var(--line-200)' }}>
                        <button onClick={t.open} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '6px 12px', border: '1px solid var(--line-300)', background: '#fff', color: 'var(--ink-700)', borderRadius: 9, fontSize: 11.5, fontWeight: 700, cursor: 'pointer' }}>
